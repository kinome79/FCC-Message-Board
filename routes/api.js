...
// Connect Mongoose Database - report when connected
...
module.exports = function (app) {
  ...
  .get( (req,res) => {
    ...
    Thread.find({board: board}).sort("-bumped_on").limit(10)
                            .select("-reported -delete_password -__v -replies.reported -replies.delete_password").exec((err, data) => {

      if (err) { return res.send(err) }  // report errors if any
  ...
  .put( (req,res) => {
    ...
    if (thread_id) {
      ...
      try {thread_id = ObjectId(thread_id);} catch(e) {return res.send("invalid id")} 

      // perform find and update on desired thread
      Thread.findOneAndUpdate({_id: thread_id, board:board}, {reported: true}, (err,obj) => {
        if (err) { return res.send(err) }  // report any errors
        if (obj) { return res.send("reported") }   // if object is returned, respond with string 'success'
        else { return res.send("invalid board/id") }   // if no object returned, respond with string 'invalid board/id'
      });
  ...
  .delete( (req,res) => {
    ...
    if (thread_id && delete_password) {
      ...
      Thread.findOne({_id: thread_id, board: board}, (err, thread) => {
        if (err) { return res.send(err) }   // print any errors that occur
        ...
        if (bcrypt.compareSync(delete_password, thread.delete_password)) {
          thread.delete( err => {
            if (err) { return res.send(err) }
            else { return res.send("success") }  // if no error, respond with string 'success'
          });
  ...
  .get( (req,res) => {
    ...
    if (thread_id) {
      ...
      Thread.findOne({board:board, _id:thread_id}).select("-reported -delete_password -__v -replies.reported -replies.delete_password").exec( (err,thread) => {
        if (err) { return res.send(err) }    // if any errors, send response with the error
        if (!thread) { return res.send("invalid board/id")}    // if no thread returned, send response stating 'invalid board/id'
  ...
  .post( (req,res) => {
    ...
    if (text && delete_password && thread_id) {
      ...
      Thread.findOne({board:board, _id: thread_id}, (err, thread) => {
        if (err) { return res.send(err) }   // if error, response with error text
        if (!thread) { return res.send("invalid board/id")}   // if no thread returned, respond with 'invalid board/id'
  ...
  .put( (req,res) => {
    ...
    if (thread_id && reply_id) {
      ...
      } catch(e) {return res.send("invalid id")}

      // retrieve desired thread object from database
      Thread.findOne({_id: thread_id, board:board}, (err,thread) => {
        if (err) { return res.send(err) }   // on error send error response
        if (!thread) { return res.send("invalid board/id") }   // if no thread found respond 'invalid board/id'
  ...
  .delete( (req,res) => {
    ...
    if (thread_id && delete_password && reply_id) {
      ...
      Thread.findOne({_id: thread_id, board: board}, (err, thread) => {
        if (err) { return res.send(err) }   // if error, respond with error
        if (!thread) { return res.send("invalid board/id")}   // if thread not found, respond 'invalid board/id'
